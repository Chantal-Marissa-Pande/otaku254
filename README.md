...
3. Copy `backend/.env.example` to `backend/.env` and add `GROQ_API_KEY`. The model defaults to `groq/compound`.
...
- Optionally set `GROQ_MODEL`; it defaults to `groq/compound`.
